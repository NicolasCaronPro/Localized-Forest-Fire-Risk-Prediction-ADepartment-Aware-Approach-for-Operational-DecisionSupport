"""Command-line interface for encoding features and clustering time series targets.

This script orchestrates the two key steps provided in the repository:
`encode`, which creates the CatBoost encodings for categorical features, and
`cluster_time_series_from_targets`, which groups departments according to their
historical target trajectories.

Both functions require a significant amount of pre-computed data.  The script
exposes configuration through command-line arguments so that the caller can
point to the correct directories and control the clustering parameters.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="List of training date strings (e.g. 2021-06-01).",
    )
    parser.add_argument(
        "--all-dates",
        nargs="+",
        required=True,
        help="Chronological list of all dates used to build the dataset.",
    )
    parser.add_argument(
        "--train-departments",
        nargs="+",
        required=True,
        help="Departments used to fit the encoder.",
    )
    parser.add_argument(
        "--cluster-departments",
        nargs="+",
        required=True,
        help="Departments considered during the clustering step.",
    )
    parser.add_argument(
        "--drop-departments",
        nargs="*",
        default=[],
        help="Departments to ignore while clustering.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    train_dates = _load_dates(args.train_dates)
    all_dates = _load_dates(args.all_dates)
    train_departments = _load_departments(args.train_departments)
    cluster_departments = _load_departments(args.cluster_departments)
    drop_departments = _load_departments(args.drop_departments)
    output_dir = args.output_dir
    target_bin_dir = args.target_bin_dir

    _validate_dates(train_dates, all_dates)
